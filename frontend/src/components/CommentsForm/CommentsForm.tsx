...
import {fetchCommentsData, postFormComments} from '../../store/comments/commentsThunks';
...
const CommentsForm: React.FC<Props> = ({id, idNews}) => {
    ...
    const onSubmitForm = async (event: React.FormEvent) => {
        try {
            ...
            if (commentsData.content.trim().length !== 0 && commentsData.newsId.length > 0) {
                ...
                await dispatch(fetchCommentsData(id)).unwrap();
                setCommentsData({
                    newsId: '',
                    author: '',
                    content: '',
